import os, sqlite3, secrets, io, csv, shutil, sys
...
def _find_base_dir():
    ...
    for c in seen:
        if os.path.isfile(os.path.join(c, "templates", "login.html")):
            return c
    # None of the candidates had templates/ - fall back to the first guess, but
    # print a clear diagnostic so a broken deploy shows an obvious cause in the
    # logs instead of a bare "TemplateNotFound: login.html" on every request.
    fallback = seen[0] if seen else os.getcwd()
    print(f"[marassi-cctv] WARNING: could not find templates/login.html near any of {seen}. "
          f"Falling back to {fallback} - make sure the 'templates' and 'static' folders are "
          f"deployed alongside app.py.", file=sys.stderr)
    return fallback
...
@app.get("/favicon.ico")
def favicon():
    return send_from_directory(os.path.join(BASE,"static","icons"),"favicon-32.png",mimetype="image/png")

@app.get("/health")
def health(): return {"status":"ok","app":"Marassi CCTV Inspection","version":"7.0"}
