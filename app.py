import os, sqlite3, secrets, io, csv, shutil, sys, base64
...
def _find_base_dir():
    # Some deployment setups run this file from a different working directory,
    # or execute it through a symlink, so a single abspath(__file__) guess can
    # end up pointing at a folder that doesn't actually contain templates/.
    # Try every plausible candidate and use the first one that really has them.
    candidates = []
    try:
    ...
    for c in candidates:
        if c and c not in seen:
            seen.append(c)
    for c in seen:
        if os.path.isfile(os.path.join(c, "templates", "login.html")):
            return c
    # None of the candidates had templates/ on disk. This isn't necessarily fatal:
    # app.py carries an embedded copy of every template/static file in assets.py as
    # a guaranteed fallback (see the ChoiceLoader/_read_static setup below), so the
    # app still works correctly even when the 'templates'/'static' folders don't
    # make it into the deployed environment. We just log this clearly so a broken
    # deploy is obvious in the logs instead of silently relying on the fallback.
    fallback = seen[0] if seen else os.getcwd()
    print(f"[marassi-cctv] NOTE: templates/login.html not found on disk near any of {seen}. "
          f"Using the embedded copies bundled in assets.py instead. If you want to edit "
          f"templates/static files directly, make sure those folders are deployed "
          f"alongside app.py at {fallback}.", file=sys.stderr)
    return fallback
...
try:
    ...
    import tempfile
    fallback_dir=os.path.join(tempfile.gettempdir(),"marassi_cctv")
    print(f"[marassi-cctv] WARNING: could not create upload folder {UPLOAD} ({e}); "
          f"using {fallback_dir} instead. Note: this directory may not persist across "
          f"restarts on some hosts - mount a persistent volume for production use.", file=sys.stderr)
    os.makedirs(fallback_dir,exist_ok=True)
    DB=os.path.join(fallback_dir,"marassi_cctv.db")
    UPLOAD=os.path.join(fallback_dir,"uploads")
    os.makedirs(UPLOAD,exist_ok=True)
...
# Always try the real files on disk first (so local edits are picked up normally);
# fall back to the embedded copies in assets.py if a template isn't on disk at all.
app.jinja_loader = ChoiceLoader([FileSystemLoader(os.path.join(BASE,"templates")), DictLoader(assets.TEMPLATES)])

def _read_static(rel_path):
    """Read a static asset (relative to the static/ folder), preferring the real
    file on disk, falling back to the embedded copy in assets.py if it's missing."""
    disk_path=os.path.join(BASE,"static",rel_path)
    if os.path.isfile(disk_path):
        with open(disk_path,"rb") as f: return f.read()
    if rel_path in assets.TEXT_ASSETS:
        return assets.TEXT_ASSETS[rel_path].encode("utf-8")
    if rel_path in assets.BINARY_ASSETS_B64:
        return base64.b64decode(assets.BINARY_ASSETS_B64[rel_path])
    return None
...
def login():
    if request.method=="POST":
        ...
        ok=False
        if u:
            is_hashed=u["password"].startswith(("scrypt:","pbkdf2:","argon2:"))
            if is_hashed:
                ok=check_password_hash(u["password"],password)
            else:
                # Legacy/plaintext password (e.g. from an older DB) - compare directly,
                # then transparently upgrade it to a proper hash on successful login.
...
def dashboard():
    ...
    # "Total requests" should always match how many rows this role can actually
    # see below (e.g. CCTV reviewers don't see Closed/Rejected requests), so we
    # derive it from the filtered rows rather than summing every status globally.
    total=len(rows)
...
def static_style():
    data=_read_static("style.css")
    if data is None: return "Not found",404
    return Response(data,mimetype="text/css")

@app.get("/static/sw.js")
def static_sw():
    data=_read_static("sw.js")
    if data is None: return "Not found",404
    return Response(data,mimetype="application/javascript")

@app.get("/static/manifest.json")
def static_manifest():
    data=_read_static("manifest.json")
    if data is None: return "Not found",404
    return Response(data,mimetype="application/manifest+json")

@app.get("/static/icons/<name>")
def static_icons(name):
    data=_read_static(f"icons/{name}")
    if data is None: return "Not found",404
    return Response(data,mimetype="image/png")

@app.get("/favicon.ico")
def favicon():
    return static_icons("favicon-32.png")

@app.get("/health")
def health(): return {"status":"ok","app":"Marassi CCTV Inspection","version":"7.2","has_embedded_assets":True}
